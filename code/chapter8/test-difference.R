...
mytest <- function(m, n, mu1, mu2, sigma){ 
...
MC <- 10000 # 実験回数
## 帰無仮説が正しい場合にMC回検定を実行
result <- replicate(MC, mytest(m, n, mu1, mu1, sigma)) 
t1 <- result[1, ] # 検定統計量の値
p1 <- result[2, ] # p値
mean(p1 < 0.05) # 有意水準5%で棄却された実験の割合
mean(p1 < 0.01) # 有意水準1%で棄却された実験の割合
## 帰無仮説が誤りの場合にMC回検定を実行
result <- replicate(MC, mytest(m, n, mu1, mu2, sigma)) 
t2 <- result[1, ] # 検定統計量の値
p2 <- result[2, ] # p値
...
ggplot(dat) +
  ...
  # x軸の範囲を-12<=x<=5にズーム
  coord_cartesian(xlim = c(-12, 5)) +
  ## 平均の差ごとに分けて描画  
  facet_wrap(~ difference, nrow = 2,
             labeller = labeller(difference = label_both)) +
  ## 帰無分布の密度関数の上書き
  stat_function(fun = \(x) dt(x, df = m + n - 2), color = "red",
                n = 1001) +
  ## 棄却域の可視化(有意水準5%)
  geom_vline(aes(xintercept = qt(0.975, df = m + n - 2)),
             color = "green", lty = "dotted") +
  geom_vline(aes(xintercept = -qt(0.975, df = m + n - 2)),
             color = "green", lty = "dotted") +
  ## 棄却域の可視化(有意水準1%)
  geom_vline(aes(xintercept = qt(0.995, df = m + n - 2)),
